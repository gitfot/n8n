...
> **WSL 环境说明：** 本项目位于 `/mnt/d/` 下，属于 WSL 环境。所有 pnpm 命令必须通过
> `powershell.exe` 调用 Windows 原生工具链执行，避免 WSL 访问 `/mnt/` 路径的 I/O 性能问题。
> 对应 Windows 项目路径为 `D:\wanwan\project\self\n8n`。

### Building
使用 `pnpm build` 构建所有包。务必将构建命令输出重定向到文件：

```bash
powershell.exe -Command "cd 'D:\wanwan\project\self\n8n'; pnpm build" > build.log 2>&1
```
...
### Testing
- `powershell.exe -Command "cd 'D:\wanwan\project\self\n8n'; pnpm test"` - 运行所有测试
- `powershell.exe -Command "cd 'D:\wanwan\project\self\n8n'; pnpm test:affected"` - 基于自上次提交以来的变更运行测试

运行某个特定测试文件时，需要进入该测试所在的包对应的 Windows 路径并执行：

```bash
# 示例：运行 packages/cli 下的某个测试
powershell.exe -Command "cd 'D:\wanwan\project\self\n8n\packages\cli'; pnpm test <test-file>"
```

### Code Quality
- `powershell.exe -Command "cd 'D:\wanwan\project\self\n8n'; pnpm lint"` - 代码规范检查
- `powershell.exe -Command "cd 'D:\wanwan\project\self\n8n'; pnpm typecheck"` - 运行类型检查

提交代码前始终运行 lint 和 typecheck，以确保质量。
请在你当前工作的具体包的 Windows 路径内执行这些命令（例如：`powershell.exe -Command "cd 'D:\wanwan\project\self\n8n\packages\cli'; pnpm lint"`）。仅在准备最终 PR 时运行全仓库检查。
